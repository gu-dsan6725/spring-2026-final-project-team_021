...
class JudgeAgent:
    ...
    def judge(
        ...
        try:
            ...
            return self._judge_with_llm(bull=bull, bear=bear, reports=reports)
        except Exception as exc:
            print(f"[{self.agent_name}] using rule fallback ({exc})")
            return self._judge_with_rules(bull=bull, bear=bear, reports=reports)
    ...
    def _judge_with_rules(
        ...
        rationale = self._build_rationale(signal=signal, bull=bull, bear=bear, reports=reports)
        dissenting_points = (
    ...
    def _build_summary(
        ...
        return f"The judge favors the {signal} case for {ticker} with confidence {confidence:.2f}."
