...
class BearAgent:
    ...
    def _build_case_with_rules(
        ...
        repeated_bear_points = self._as_list(latest_round.get("repeated_bear_points"))
        ...
        if signal_history.count("bearish") >= 2:
            aligned_support += 0.3
            supporting_evidence.append(
                "[Memory] Recent judge history has leaned bearish, which supports continuity if the current downside evidence remains active."
            )

        if recurring_risks:
            aligned_support += min(len(recurring_risks), 3) * 0.1
            supporting_evidence.extend(
                f"[Memory] Recurring downside risk across recent weeks: {risk}" for risk in recurring_risks[:3]
            )
            risk_flags.extend(recurring_risks[:3])

        if repeated_bear_points:
            opposing_pressure += min(len(repeated_bear_points), 2) * 0.08
            counter_evidence.extend(
                f"[Memory] Earlier bear round already relied on: {point}" for point in repeated_bear_points[:2]
            )
