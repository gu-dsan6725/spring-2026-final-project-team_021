...
1. **Price (Yahoo Finance)** — daily OHLCV for each ticker → used by the Technical Analyst (`price_collector.py`)
2. **Fundamentals (SEC EDGAR + Yahoo Finance)** — quarterly financial statements and derived ratios → used by the Fundamental Analyst (`fundamental_collector.py`)
3. **Google Trends (pytrends)** — weekly search interest as retail attention proxy → used by the News & Trend Analyst (`google_trends_collector.py`)
4. **News (Finnhub)** — company news headlines → used by the News & Trend Analyst (`finnhub_news_fetch.py`)
5. **Macro (FRED)** — 35 macroeconomic indicators → used by the Macro Analyst (`fred_macro_fetch.py`)
